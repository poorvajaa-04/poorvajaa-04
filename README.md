# I'm Poorvajaa S  

🚀 Cybersecurity Enthusiast | Networking & Systems Learner  
🔐 Interested in building security tools, analyzing systems, and solving real-world problems  

I’m currently focused on strengthening my foundations in **networking, Linux, and security**, while building projects to apply what I learn.

---

## Tech Stack  

### Programming  
- Python  
- C++ 
- C 
- JavaScript  

### Web  
- HTML 
- CSS 
- Next.js  

### Systems & Tools  
- Linux 
- Wireshark 
- Git 
- VS Code 
- Vercel  

### Data  
- SQL 
- Excel  

---
## 📂 Projects  

### 🔹 KalaQuest (Hackathon Project)  
A platform designed to **connect artisans directly with customers**, reducing reliance on middlemen.  

- Built using **Next.js, JavaScript, Python**  
- Developed core features for showcasing artisan products  
- Focused on accessibility and digital visibility for local creators  

👉 *Next Step:* Improving scalability & adding secure transaction features  

---
## 🧠 Current Focus  

- 🌐 Computer Networking (OSI, TCP/IP, DNS, HTTP/HTTPS)  
- 🐧 Linux & System Internals  
- 🔐 Cybersecurity Fundamentals  
- 🔧 Git, GitHub & Open Source  

---

## Goals  

-  Contribute to Open Source (GSSoC)  
-  Build beginner-friendly cybersecurity tools  
-  Develop strong networking & system fundamentals  

---

## 📈 GitHub Stats  

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=poorvajaa-04&bg_color=transparent&color=555&line=4C8EDA&point=888&hide_border=true"/>
</p>

## 📊 Contribution Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=poorvajaa-04&bg_color=transparent&color=555&line=4C8EDA&point=888&hide_border=true"/>
</p>

---

## Connect with Me  

-  LinkedIn: https://www.linkedin.com/in/poorvajaa-s-286a50398/  
-  Email: poorvajaa.16@gmail.com  
